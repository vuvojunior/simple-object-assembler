...
public abstract class GenericTypeResolver {
   ...
   public static Class getParameterizedTypeByName(String typeName, Class clazz) {

      final Map typeVariableMap = getTypeVariableMap(clazz);

      for (Object object : typeVariableMap.entrySet()) {
         Entry entry = (Entry) object;
         if (((TypeVariable) entry.getKey()).getName().equals(typeName)) {
    ...
    * 
    * public class FooImpl implements Foo<String, Integer> { .. }</pre> For '
    * <code>FooImpl</code>' the following mappings would be added to the
      ...
      if (type.getRawType() instanceof Class) {
         ...
         for (int i = 0; i < actualTypeArguments.length; i++) {
            ...
            if (actualTypeArgument instanceof Class) {
               ...
               // the nearest match on the bridge method will be the bounded
               // type.
